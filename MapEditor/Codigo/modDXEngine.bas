...
End Function
Private Function Particles_GroupUpdate(ByVal ParticleGroup_Index As Integer, ByVal X As Integer, ByVal y As Integer)
    Dim i As Integer
    Dim Time As Long
                    ...
                    For i = 0 To .Particle_count
                        ...
                        If .Particle(i).Alive = 0 And Not .Particle(i).Used Then
                            ...
                            Else
                                ...
                                .Particle(i).rgb_list(0) = D3DColorARGB(.Particle(i).CurrentColor(1).A, .Particle(i).CurrentColor(1).R, .Particle(1).CurrentColor(1).G, .Particle(i).CurrentColor(1).B)
                                .Particle(i).rgb_list(1) = D3DColorARGB(.Particle(i).CurrentColor(2).A, .Particle(i).CurrentColor(2).R, .Particle(2).CurrentColor(2).G, .Particle(i).CurrentColor(2).B)
                                .Particle(i).rgb_list(2) = D3DColorARGB(.Particle(i).CurrentColor(3).A, .Particle(i).CurrentColor(3).R, .Particle(3).CurrentColor(3).G, .Particle(i).CurrentColor(3).B)
                                .Particle(i).rgb_list(3) = D3DColorARGB(.Particle(i).CurrentColor(4).A, .Particle(i).CurrentColor(4).R, .Particle(4).CurrentColor(4).G, .Particle(i).CurrentColor(4).B)
                        ...
                        Else
                            If Not .Particle(i).Used Then
                                Call Particle_Update(ParticleGroup_Index, i, X, y, Time)
                            End If
                        End If
                    Next i
                Else
    ...
    End With
End Function
Public Sub DXEngine_ParticleGroupRender(ByVal particle_group_index As Integer, ByVal X As Integer, ByVal y As Integer)
    Dim i As Integer
    Dim rgb_list(3) As Long
    Dim Size As Integer
    
    Call Particles_GroupUpdate(particle_group_index, X, y)
    
    With particle_group_list(particle_group_index).ParticleEmisor
        For i = 1 To .Particle_count
            'If is destroyed...
            If .Particle(i).Used = False Then
...
Private Sub LoadParticles()
    ...
    If Not General_File_Exist(particle_path, vbArchive) Then Exit Sub
    
    particle_group_data_count = Val(general_var_get(particle_path, "INIT", "Total"))
            ...
            With .ParticleEmisor
                ...
                If .ColorVariation Then
                    For j = 1 To 4
                        ...
                        .StartColor(j) = D3DColorValueGet(Val(general_field_read(1, aux, Asc(","))), Val(general_field_read(4, aux, Asc(","))), general_field_read(3, aux, Asc(",")), Val(general_field_read(2, aux, Asc(","))))
                        aux = general_var_get(particle_path, i, "ColorEnd" & j)
                        .EndColor(j) = D3DColorValueGet(Val(general_field_read(1, aux, Asc(","))), Val(general_field_read(4, aux, Asc(","))), Val(general_field_read(3, aux, Asc(","))), Val(general_field_read(2, aux, Asc(","))))
                    Next j
                Else
                    For j = 1 To 4
                        aux = general_var_get(particle_path, i, "ColorSet" & j)
                        .StartColor(j) = D3DColorValueGet(Val(general_field_read(1, aux, Asc(","))), Val(general_field_read(4, aux, Asc(","))), Val(general_field_read(3, aux, Asc(","))), Val(general_field_read(2, aux, Asc(","))))
                    Next j
                End If
...
Private Sub Particle_Update(ByVal GroupIndex As Integer, ByVal i As Integer, ByVal X As Integer, ByVal y As Integer, ByVal Time As Long)
    ...
    With particle_group_list(GroupIndex)
        With .ParticleEmisor
            ...
            'Move our particle
            .Particle(i).Moved_X = .Particle(i).Moved_X + .Particle(i).vX \ .Friction
            .Particle(i).Moved_Y = .Particle(i).Moved_Y + .Particle(i).vY \ .Friction
...
Private Function ParticleGroup_CheckPermanency(ByVal ParticleGroup_Index As Integer, ByVal Time As Long) As Boolean
    ...
    With particle_group_list(ParticleGroup_Index)
        If .ParticleGroup_Lifetime > 0 Or .ParticleGroup_Lifetime = -1 And Not .ParticleEmisor.StartParticlesDestroy Then
