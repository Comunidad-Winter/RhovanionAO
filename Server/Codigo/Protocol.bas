...
'On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.SaaoClientPacket
            Call HandleSaaoClientPacket(UserIndex)
...
#If SeguridadAlkon Then
    ...
    End If
#Else
    If UserList(UserIndex).incomingData.length < 54 Then
        Err.raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
        Exit Sub
    End If
#End If
...
'#End If
        ...
        Else
            Call ConnectNewUser(UserIndex, UserName, Password, race, gender, Class, skills, mail)
        End If
'#If SeguridadAlkon Then
    End If
'#End If
    ...
    With UserList(UserIndex)
        ...
        Select Case Skill
            Case eSkill.Proyectiles
                'FIJARSE SI SE PUEDE REEMPLAZAR EL SISTEMA DE INTERVALOS POR EL LAC
...
Public Sub WriteRetoRequest(ByVal UserIndex As Integer)
    UserList(UserIndex).outgoingData.WriteByte (ServerPacketID.Reto)
End Sub
...
End Sub
Public Sub HandleSaaoClientPacket(ByVal UserIndex As Integer)
...
'On Error Resume Next
    ...
    Select Case packetID
        ...
        Case MiClientPacketID.Reto
            Call HandleReto(UserIndex)
        Case MiClientPacketID.Acepto
            Call HandleAceptoReto(UserIndex)
        Case MiClientPacketID.NoAcepto
            Call HandleNoAceptoReto(UserIndex)
        Case MiClientPacketID.Subastar
