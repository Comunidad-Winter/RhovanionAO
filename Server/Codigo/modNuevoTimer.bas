...
Public Function IntervaloPermiteGolpeMagia(ByVal UserIndex As Integer, Optional ByVal Actualizar As Boolean = True) As Boolean
    ...
    End If
End Function
Public Function IntervaloPermitePotear(ByVal UserIndex As Integer, Optional ByVal Actualizar As Boolean = True) As Boolean
Dim TActual As Long

TActual = GetTickCount() And &H7FFFFFFF

If TActual - UserList(UserIndex).Counters.TimerPuedePotear >= IntervaloUserPuedePotear Then
    If Actualizar Then
        UserList(UserIndex).Counters.TimerPuedePotear = TActual
    End If
    IntervaloPermitePotear = True
Else
    IntervaloPermitePotear = False
End If
End Function
